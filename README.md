...
- **Add** and **edit** entries with:
  ...
  - **Attachment** (PDF or image(s)): normalized to an A4 PDF — **JPEG** streams (PyMuPDF embeds DCT; long edge **2000 px**, quality **75**). You can select **multiple photos** at once (one PDF page per photo, max **40** pages total). **Multiple files must be images only**; a PDF must be uploaded alone. Single PDFs may have up to **40** pages.
